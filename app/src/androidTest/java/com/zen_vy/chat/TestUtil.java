...
public class TestUtil {
    ...
    public static User addUser(User user, Context context){
        UserService userService = new UserService(context);
        userService.addUser(user,  new UserService.LoginCallback<User>() {
            @Override
...
user.setPublicKey(RandomUtil.getRandomString(6));
    ...
    public static void performLogin(Context context) throws InterruptedException {
        UserService userService = new UserService(context);
        userService.getTokenByPasswordAndEmail(TEST_PASSWORD, TEST_EMAIL, new UserService.LoginCallback<User>() {
            @Override
            public void onSuccess(User data) {
                   SharedPreferencesUtil.setStringPreference(context, SharedPreferencesConstants.USERTOKEN, data.getToken());

                    SharedPreferencesUtil.setStringPreference(context, SharedPreferencesConstants.UUID, data.getUuid());
            }
