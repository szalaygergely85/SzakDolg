...
public class MessageService extends BaseService {
   ...
   public void updateMessage(MessageEntry entry) {
      ...
      MessageEntry existingEntry = messageDatabaseUtil.getMessageByUuid(entry.getUuId());
      ...
      if (existingEntry != null) {
         ...
         if (entry.getMessageId() != null && !entry.getMessageId().equals(existingEntry.getMessageId())) {
            updatedEntry.setMessageId(entry.getMessageId());
         } else {
            updatedEntry.setMessageId(existingEntry.getMessageId());
         }

         if (entry.getConversationId() != null && !entry.getConversationId().equals(existingEntry.getConversationId())) {
            updatedEntry.setConversationId(entry.getConversationId());
         } else {
            updatedEntry.setConversationId(existingEntry.getConversationId());
         }

         if (entry.getSenderId() != null && !entry.getSenderId().equals(existingEntry.getSenderId())) {
            updatedEntry.setSenderId(entry.getSenderId());
         } else {
            updatedEntry.setSenderId(existingEntry.getSenderId());
         }

         if (entry.getTimestamp() != null && !entry.getTimestamp().equals(existingEntry.getTimestamp())) {
            updatedEntry.setTimestamp(entry.getTimestamp());
         } else {
            updatedEntry.setTimestamp(existingEntry.getTimestamp());
         }

         if (entry.getContentEncrypted() != null && !entry.getContentEncrypted().equals(existingEntry.getContentEncrypted())) {
            updatedEntry.setContentEncrypted(entry.getContentEncrypted());
         } else {
            updatedEntry.setContentEncrypted(existingEntry.getContentEncrypted());
         }
         ...
         if (entry.getContent() != null && !entry.getContent().equals(existingEntry.getContent())) {
