...
public class ConversationParticipantApiHelper extends BaseService {
    ...
    public void getParticipants(Long conversationId, Consumer<List<ConversationParticipant>> onSuccess) {
        Call<List<ConversationParticipant>> call = conversationParticipantApiService.getConversationParticipants(conversationId, currentUser.getAuthToken());
        call.enqueue(
                    ...
                    public void onResponse(
                        ...
                        if (response.isSuccessful()) {
                            if (response.body().size() > 0) {
                                List<ConversationParticipant> participants = response.body();
