...
public class ImageRepositoryImpl implements ImageRepository {
    ...
    public ImageRepositoryImpl(Context context, User currentUser) {
        ...
        this.imageApiService = RetrofitClient.getRetrofitInstance().create(ImageApiService.class);
        imageDatabaseUtil = new ImageDatabaseUtil(context, currentUser);
    }

    @Override
    public void uploadImage(MultipartBody.Part filePart, ImageEntity imageEntity, String token, Callback<ResponseBody> callback) {
        imageApiService.uploadImage(filePart, imageEntity, token).enqueue(new Callback<ResponseBody>() {
            @Override
    ...
    @Override
    public void downloadImage(String uuid, String token, Callback<ResponseBody> callback) {
