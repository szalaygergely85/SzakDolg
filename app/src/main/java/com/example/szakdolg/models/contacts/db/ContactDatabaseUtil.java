...
public class ContactDatabaseUtil {
    ...
    public ContactDatabaseUtil(Context context, User currentUser) {
        this.dbHelper = DatabaseHelper.getInstance(context, currentUser.getUserId().toString());
    }
    ...
    public void deleteContact(Long contactId) {
        try (SQLiteDatabase db = dbHelper.getWritableDatabase()) {
            db.delete("Contact", "contactId = ?", new String[]{String.valueOf(contactId)});
        }
    }
    ...
    public void updateContact(Contact contact) {
        try (SQLiteDatabase db = dbHelper.getWritableDatabase()) {
            ...
            db.update("Contact", values, "contactId = ?", new String[]{String.valueOf(contact.getContactId())});
        }
    }
    ...
    public Contact getContact(Long contactId) {
        ...
        try (SQLiteDatabase db = dbHelper.getReadableDatabase();
            ...
            if (cursor.moveToFirst()) { // Only fetch one contact
                Long ownerId = cursor.getLong(cursor.getColumnIndexOrThrow("ownerId"));
                Long contactUserId = cursor.getLong(cursor.getColumnIndexOrThrow("contactUserId"));
    ...
    public List<Contact> getContacts(String searchQuery) {
        ...
        try (SQLiteDatabase db = dbHelper.getReadableDatabase();
            ...
            while (cursor.moveToNext()) {
                Long contactId = cursor.getLong(cursor.getColumnIndexOrThrow("contactId"));
                Long ownerId = cursor.getLong(cursor.getColumnIndexOrThrow("ownerId"));
                Long contactUserId = cursor.getLong(cursor.getColumnIndexOrThrow("contactUserId"));
    ...
    public void insertContacts(List<Contact> contacts) {
        ...
        try {
            ...
            for (Contact contact : contacts) {
                ...
                values.put("ownerId", contact.getOwnerId());
                values.put("contactUserId", contact.getContactUserId());
                db.insertWithOnConflict("Contact", null, values, SQLiteDatabase.CONFLICT_REPLACE);
            }
