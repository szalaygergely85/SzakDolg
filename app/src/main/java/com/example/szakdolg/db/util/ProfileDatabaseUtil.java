...
public class ProfileDatabaseUtil {
    ...
    public ProfileDatabaseUtil(Context context) {
    ...
    public User getCurrentUserByToken(String token) {
        ...
        try {
            ...
            String query = "SELECT * FROM "+dbHelper.TABLE_PROFILE+ " WHERE token = ?;";
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                currentUser = new User(userID, displayName, fullName, email, phoneNumber);
            }
        } finally {
            if (cursor != null) {
                cursor.close();
